...
class ThemeController extends Controller
{
    ...
    public function update($slug)
    {
        if (file_exists(base_path('../themes/' . $slug) . '/theme.json')) {
            ...
            return redirect()->back()
                ->with('alert', 'success')
                ->with('message', __('lang.theme_changed'));
        }

        return redirect()->back()
            ->with('alert', 'danger')
            ->with('message', __('lang.theme_not_found'));
    }
    ...
    public function installFromSource(Request $request)
    {
            ...
            if (count($dirs) > 2) {
                ...
                if (isset($theme['slug']) && isset($theme['name']) && isset($theme['version']) && isset($theme['author'])) {
                    if (file_exists(base_path('../themes/' . $dirs[2]))) {
                        ...
                        return redirect()->back()
                            ->with('alert', 'danger')
                            ->with('message', __('lang.theme_already_installed'));
                    }
                    ...
                    return redirect()->back()
                        ->with('alert', 'success')
                        ->with('message', __('lang.theme_installed'));
                }
            }
        } catch (\Exception $e) {
            $this->rollbackInstallFromSource();
        ...
        return redirect()->back()
            ->with('alert', 'danger')
            ->with('message', __('lang.theme_not_installed'));
    }
