...
@section('page-title'){{ __('lang.dashboard') }}@endsection
                ...
                <div class="card-header">
                    ...
                    <div class="btn-group float-left btn-group-sm">
                        <button type="button" class="btn @if(site_config('live_stats')) btn-success @else btn-secondary @endif dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            {{ __('lang.live_stats') }}: @if(site_config('live_stats')) {{ __('lang.active') }} @else {{ __('lang.deactive') }} @endif
                        </button>
                        <div class="dropdown-menu">
                            @if(site_config('live_stats'))
                                <a class="dropdown-item" href="{{ route('admin.dashboard.live.toggle') }}">{{ __('lang.deactivate') }}</a>
                            @else
                                <a class="dropdown-item" href="{{ route('admin.dashboard.live.toggle') }}">{{ __('lang.activate') }}</a>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        if (transactionsChart) {
          ...
          transactionsChart = new Chart(transactionsChartEl, {
            ...
            data: {
              labels: labels,
              datasets: [{
                label: "{{ __('lang.transactions_count') }}",
                data: values,
                backgroundColor: [
                  'rgba(54, 162, 235, 0.2)',
                ],
